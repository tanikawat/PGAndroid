...
function startUpdate(){
  ...
  $.getJSON("http://app.businessmap.com.br/json/users/",function(result){
    $.each(result, function(i, field){
      ...
      {
        //tx.executeSql('INSERT INTO user(nome,login,password) VALUES ("Tulio Tanikawa","Tulio","1")');
        tx.executeSql('INSERT INTO user(idUser,nome,login,password) VALUES ("'+field.id+'","'+field.name+'","'+field.email+'","'+field.senhaapp+'")');
      });
    });
  });
  //alert("deu boa");
...
function updateChat(){
  ...
  $.getJSON("http://app.businessmap.com.br/chat/list/"+idUsuario,function(result){
    $.each(result, function(i, field){
      $( "#incomingMessages" ).append('<div class="ui-grid-a"><div class="ui-block-a"><div class="ui-body ui-body-a ui-body-d ui-corner-all"><h5>Mensagem da Gerência:</h5><p>'+ field.mensagem+ '</p><p>'+field.data +'</p></div><div class="ui-block-c"></div></div><p></p><p></p>');
      $.getJSON("http://app.businessmap.com.br/chat/arquivar/"+field.id,function(result){ });
    });
  });


  var mensagemUsuariovar = $('#mensagemUsuario').val();
  $('#mensagemUsuario').val("");
  $.ajax({
    type: 'POST',
    url: "http://app.businessmap.com.br/chat/new/",
    data: {'de_user_id':idUsuario,'para_user_id':2,'mensagem':mensagemUsuariovar},
    success: function(responseData, textStatus, jqXHR) {
      //alert("foi");
      var date2 = new Date();
      ...
      var min2 = ('0'+date2.getMinutes()).substr(-2);
      var seconds2 = ('0'+date2.getSeconds()).substr(-2);
      var hour2 = ('0'+date2.getHours()).substr(-2);
      var dataTeste2= day2+"-"+month2+"-"+year2+" "+hour2+":"+min2;
      $( "#incomingMessages" ).append('<div class="ui-grid-a"><div class="ui-block-a"></div><div class="ui-block-b"><div class="ui-body ui-body-b ui-body-d ui-corner-all"><h5>'+nomeCompleto+':</h5><p>'+mensagemUsuariovar+'</p>'+dataTeste2+'</div></div></div>');        },
  ...
  function userChatHistory(){
    //alert("pelo menos entrei aca hehe e id usuario é > "+idUsuario);
    $.getJSON("http://app.businessmap.com.br/chat/desarquivar/"+idUsuario,function(result){});
    $.getJSON("http://app.businessmap.com.br/chat/list/"+idUsuario,function(result){
      $.each(result, function(i, field){
        $( "#incomingMessages" ).append('<div class="ui-grid-a"><div class="ui-block-a"><div class="ui-body ui-body-a ui-body-d ui-corner-all"><h5>Mensagem da Gerência:</h5><p>'+ field.mensagem+ '</p><p>'+field.data+'</p></div><div class="ui-block-c"></div></div>');
        $.getJSON("http://app.businessmap.com.br/chat/arquivar/"+field.id,function(result){ });
      });
    });
  }
